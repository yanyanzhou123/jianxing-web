...
### 运营后台 `/ops/`
- **两级编辑**：一级只管理「章节 → 课 → 小节」结构（增删改、拖拽排序）；进入小节后才编辑文字 / 上传音视频
- **上传体验**：进度显示在对应上传控件旁；上传成功后**自动保存目录**；有未保存的结构/文字修改时显示提示条
- **已上传预览**：小节页可预览音频 / 视频；结构列表用文/音/视标记状态
...
### 工程与部署
- 目录版本升级为 **v3**（顶层 `references` + `modules`）；旧数据读取时会自动把模块内参考资料提升到顶层
...
└── modules[]             # 大模块
    └── chapters[]
        └── lessons[]
            └── segments[]   # 文字 / audioPath / videoPath
```
...
读完本 README 与 `运营说明.md` 即可了解 **v1.1** 当前状态。线上目录与媒体在 R2，不在本仓库二进制里。
